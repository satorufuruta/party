...
var clearQuizUserCookie = /* @__PURE__ */ __name(() => createCookie("", { maxAge: 0 }), "clearQuizUserCookie");
...
var redirectResponse = /* @__PURE__ */ __name((request, status, cookies) => {
  const url = new URL(request.url);
...
var worker = {
  async fetch(request, env2) {
    ...
    if (url.pathname.startsWith("/api/sessions/")) {
      return await handleSessionApi(request, env2);
    }
...
var QuizRoomDurableObject = class {
  ...
  async onMessage(context2, event) {
    ...
    switch (message.type) {
      ...
      case "submit_answer":
        ...
        break;
      case "heartbeat":
        this.handleHeartbeat(context2, message);
        break;
      case "admin_control":
  ...
  handleSyncRequest(context2) {
    ...
    const isParticipantClient = context2.role === "participant" && Boolean(context2.userId);
    const participants = isParticipantClient ? allParticipants.filter((participant) => participant.userId === context2.userId) : allParticipants;
    if (isParticipantClient && participants.length === 0 && context2.userId) {
  ...
  serializeParticipantAnswersForSignature(answers) {
    if (!answers) {
      return [];
    }
    return Object.entries(answers).sort(([a], [b]) => a.localeCompare(b)).map(([questionId, answer]) => [
      questionId,
      answer.choiceId,
      answer.submittedAt,
      answer.isCorrect === void 0 ? null : answer.isCorrect ? 1 : 0
    ]);
  }
  participantSignatureForClient(participant) {
    if (!participant) {
      return "missing";
    }
  ...
  adminParticipantsSignature() {
    return Object.values(this.snapshot.participants).map((participant) => this.participantSignatureForClient(participant)).sort().join("|");
  }
  pendingResultsSignature() {
    return Object.entries(this.snapshot.pendingResults).sort(([a], [b]) => a.localeCompare(b)).map(
      ([questionId, summary]) => JSON.stringify([
        questionId,
        Object.entries(summary.totals).sort(([left], [right]) => left.localeCompare(right)),
        [...summary.correctChoiceIds].sort()
      ])
    ).join("|");
  }
  computeClientStateSignature(context2) {
    const base = [
      context2.role ?? "guest",
    ...
    if (context2.role === "participant" && context2.userId) {
      base.push(this.participantSignatureForClient(this.snapshot.participants[context2.userId]));
    } else if (context2.role === "admin") {
  ...
  handleHeartbeat(context2, message) {
    if (context2.userId) {
      const participant = this.snapshot.participants[context2.userId];
      if (participant) {
        participant.lastSeen = now();
    ...
    const serverSignature = this.computeClientStateSignature(context2);
    ...
    this.handleSyncRequest(context2);
    const updatedSignature = this.computeClientStateSignature(context2);
    this.send(context2, {
      type: "sync_ack",
      sessionId: this.snapshot.sessionId,
      timestamp: now(),
...
// .wrangler/tmp/bundle-j4Cowm/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-j4Cowm/middleware-loader.entry.ts
